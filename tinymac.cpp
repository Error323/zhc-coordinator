...
#undef DEBUG
#define WITH_TINYMAC_COORDINATOR 1
...
static void tinymac_rx_registration_request(tinymac_header_t *hdr, size_t size)
{
  ...
  if (node)
  {
    INFO("Registered node %02X for %016 with flags %04X\n", node->addr,
         attach->uuid, attach->flags);
    node->state = tinymacNodeState_Registered;
                                              ...
                                              size_t size)
{
  ...
  if (!node || node->uuid != detach->uuid)
  {
    ...
    ERROR("Bad deregistration request from %016\n", detach->uuid);
    return;
  }
  ...
  /* Free the slot */
  INFO("De-registered node %02X for %016 reason %u\n", hdr->src_addr,
      ...
      hdr->src_addr != TINYMAC_ADDR_UNASSIGNED)
  {
    ...
    else
    {
      ...
      if (tinymac_ctx->params.coordinator)
      {
        ...
        dummy.addr = hdr->src_addr;
        dummy.state = tinymacNodeState_Registered;
...
{
  memcpy(&(tinymac_ctx_.params), params, sizeof(tinymac_params_t));
